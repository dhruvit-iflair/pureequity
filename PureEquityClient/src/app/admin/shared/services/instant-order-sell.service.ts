...
export class InstantOrderSellService {
     ...
     */
    sell(amount: any, receive: any, sellActual: any) {
        if (amount > 0) {
            ...
            this.callHttp(payload).subscribe((sellInstant: any) => {
                this.coinTransaction(amount,sellInstant.payload.data,'IOS','success').subscribe((cTSuccess: any) => {
                    console.log("cTSuccess",cTSuccess);
                    this.coinService.refreshCoinTransaction();
                },(er:any)=>{
                    console.log(er);
                });
                this.moneyTransaction(amount,sellInstant.payload.data,'IOS','success').subscribe((mTSuccess: any) => {
    ...
    callHttp(payload: any) {
        return this.http.post(environment.tradingApi + `/sellInstant/` + this.activeTrade.value, payload)
    }
